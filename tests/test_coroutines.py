import datetime
import unittest

from core import Point
from coroutines import initial_breakout, stop_loss, stop_profit, time_based

class TestCoroutines(unittest.TestCase):

    def _next_point( self, p, dt=None ):
        ''' helper function to generate data points for testing '''
        if dt is None:
            dt = self.dt

        point = Point( dt, p )
        self.dt = dt + datetime.timedelta(minutes=1)
        return point

    def setUp( self ):
        self.dt = datetime.datetime( 2020, 4, 6, 9, 30 )

    def test_initial_breakout( self ):
        cr = initial_breakout( 3 )

        # set initial range
        cr.send( (self._next_point( 50.00 ), None) )
        cr.send( (self._next_point( 50.25 ), None) )
        cr.send( (self._next_point( 50.10 ), None) )

        # no breakout
        for p in (50.05, 50.00, 50.25, 49.00):
            signal = cr.send( (self._next_point( p ), None) )
            self.assertIsNone( signal )

        # breakout
        signal = cr.send( (self._next_point( 50.26 ), None) )
        self.assertIsNotNone( signal )

        # no breakout
        signal = cr.send( (self._next_point( 50.25 ), None) )
        self.assertIsNone( signal )

        # current code doesn't allow re-triggering ...
        for p in ( 51.00, 52.00, 53.00, 54.00, 55.00 ):
            signal = cr.send( (self._next_point( 50.27 ), None) )
            self.assertIsNone( signal )

        # if we get to the next day, the coroutine should reset and start working again
        self.dt = datetime.datetime( 2020, 4, 7, 9, 30 )

        # set initial range
        cr.send( (self._next_point( 50.00 ), None) )
        cr.send( (self._next_point( 50.25 ), None) )
        cr.send( (self._next_point( 50.10 ), None) )

        # no breakout
        for p in (50.05, 50.00, 50.25, 49.00):
            signal = cr.send( (self._next_point( p ), None) )
            self.assertIsNone( signal )

        # breakout
        signal = cr.send( (self._next_point( 50.26 ), None) )
        self.assertIsNotNone( signal )

    def test_stop_loss( self ):
        cr = stop_loss( 0.01 )

        # send first point to set the stop level
        cr.send( (self._next_point( 100.00 ), None) )

        # no level breached
        for p in ( 100.00, 100.50, 102.00, 99.00 ):
            signal = cr.send( (self._next_point( p ), None) )
            self.assertIsNone( signal )

        # stop triggered
        signal = cr.send( (self._next_point( 98.99 ), None) )
        self.assertIsNotNone( signal )

        # stop should not be triggered again!
        signal = cr.send( (self._next_point( 98.99 ), None) )
        self.assertIsNone( signal )

    def test_stop_profit( self ):
        cr = stop_profit( 0.01 )

        # send first point to set the stop level
        cr.send( (self._next_point( 100.00 ), None) )

        # no level breached
        for p in ( 100.00, 100.50, 98.00, 101.00 ):
            signal = cr.send( (self._next_point( p ), None) )
            self.assertIsNone( signal )

        # stop triggered
        signal = cr.send( (self._next_point( 101.01 ), None) )
        self.assertIsNotNone( signal )

        # stop should not be triggered again!
        signal = cr.send( (self._next_point( 101.01 ), None) )
        self.assertIsNone( signal )

    def test_time_based( self ):
        cr = time_based( 9, 34 )

        # no signal
        for p in ( 100.00, 100.50, 102.00, 99.00 ):
            signal = cr.send( (self._next_point( p ), None) )
            self.assertIsNone( signal )

        # time triggered
        signal = cr.send( (self._next_point( 99.50 ), None) )
        self.assertIsNotNone( signal )

        # no more triggerring because we're matching exactly
        signal = cr.send( (self._next_point( 99.50 ), None) )
        self.assertIsNone( signal )

if __name__ == '__main__':
    unittest.main()